...
public class MysqlCheckServiceImpl implements MysqlCheckService {
    ...
    public void checkDataBaseNameSafe(String userSchemaName) throws NormalErrorException {
        ...
        ThrowUtils.throwIf(!userSchemaName.matches("[A-Za-z0-9_]+"), ErrorEnum.paramNotSupport,"数据库名只支持英文字母和数字、下划线");
        ThrowUtils.throwIf(userSchemaName.length() > 30, ErrorEnum.paramNotSupport, "数据库名过长！");
    }
    ...
    public boolean checkDatabaseExist(String actionID, String userSchemaName, boolean strict) throws NormalErrorException {
        ...
        if (strict && empty) {
            throw new NormalErrorException(ErrorEnum.NotExist, "查询的数据库" + userSchemaName + "不存在！");
        }
        return !empty;
    }
